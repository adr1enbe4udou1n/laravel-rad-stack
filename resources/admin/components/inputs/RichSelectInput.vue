  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const classes = computed(() => {
        return {
          container:
            'relative mx-auto w-full flex items-center justify-end box-border cursor-pointer border border-gray-300 rounded shadow-sm bg-white text-base leading-snug outline-none',
          ...
          multipleLabel:
            'flex items-center h-full absolute left-0 top-0 pointer-events-none bg-transparent leading-snug pl-3.5',
          search:
            'flex items-center h-full absolute left-0 top-0 w-full outline-none appearance-none box-border border-0 text-base font-sans bg-white rounded pl-3.5',
          tags: 'flex-grow flex-shrink flex flex-wrap mt-1 pl-2',
          tag: 'bg-green-500 text-white text-sm font-semibold py-0.5 pl-2 rounded mr-1 mb-0.5 flex items-center whitespace-nowrap',
          ...
          caret:
            'bg-multiselect-caret bg-center bg-no-repeat w-5 h-5 py-px box-content mr-3.5 relative z-10 opacity-40 flex-shrink-0 flex-grow-0 transition-transform transform',
